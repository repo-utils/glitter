...
Glitter.prototype.install = function () {
  ...
  if (progress[hash]) return await(hash)
  progress[hash] = true
  return exec('git', ['clone', '--recursive', this.url, this.folder]).then(fin, fin)
...
Glitter.prototype.update = function () {
  ...
  if (progress[hash]) return await(hash)
  progress[hash] = true
  return exec('git', ['fetch', '-f', '--all'], this.execopts).then(parseFetch).then(fin, fin)

  function fin(val) {
    delete progress[hash]
    ee.emit(hash, val)
    if (val instanceof Error) throw err
    return val
  }
}
