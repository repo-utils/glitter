...
Glitter.prototype.show = function (reference, remote) {
  var self = this
  var promise = exec('git', ['show', '--quiet', reference], remote ? this.remoteexecopts : this.localexecopts).then(parseShowHash)
...
Glitter.prototype.getReferences = function (reference, remote) {
  ...
  debug('cmd: %o', args)
  return exec('git', args, remote ? this.remoteexecopts : this.localexecopts).then(parseReferences)
}
